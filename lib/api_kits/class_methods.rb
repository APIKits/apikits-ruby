module ApiKits
  ...
  module ClassMethods
    ...
    %w(get delete).each do |method|
      class_eval <<-EVAL
        def #{method}(id, header = {})
          return new(:id => id) if ApiKits.config.mock
          url = "\#{ApiKits.config.api_uri}\#{self.resource_path}/\#{id}"
    ...
    %w(put patch).each do |method|
      class_eval <<-EVAL
        def #{method}(id, attributes, header = {})
          ...
          url = "\#{ApiKits.config.api_uri}\#{self.resource_path}/\#{id}"
          response = ApiKits::Dispatcher.send('#{method}', url, { self.root_node.to_sym => attributes }, header)
          build(response, url)
        end
      EVAL
    end
    ...
    # Make a post request and initialize an object with the response.
    #
